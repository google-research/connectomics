...
def train_flow_matching(
  ...
  def _get_dynamic_batch(state_step, batch, config):
    ...
    if config.dynamic_batch_freq > 0:
      is_regular_step = state_step % config.dynamic_batch_freq == 0
  ...
  def train_step(state, batch):
    ...
    cur_batch, cur_n_points = _get_dynamic_batch(state.step, batch, config)
